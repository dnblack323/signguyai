...
- Base URL: `https://prod-key-update.preview.emergentagent.com`
...
- ❌ **1.1B** Backup size > 50KB (observed: `32944 bytes`)
- ❌ **1.1C** Required collection-name set match (missing expected names: `orders`, `order_items`, `payroll_transactions`, `timeclock_shifts`)
...
- ❌ **1.3A** Subscription endpoint did not expose renewal/trial date as expected in this check (`plan=free_trial`, `current_period_end=None`, `trial_end=None`)
...
- ❌ **1.6D** Empty import payload did not error (returned 200 with created=0)
...
- ❌ **1.6K** Phone format search failed (`(415) 555-1234`)
- ❌ **1.6L** Phone format search failed (`415.555.1234`)
- ❌ **1.6M** Phone format search failed (`415-555-1234`)
- ❌ **1.6N** Phone format search failed (`+1 415 555 1234`)
- ❌ **1.6O** Phone format search failed (`+14155551234`)
- ❌ **1.6P** Invalid email row was not skipped (both rows imported)
