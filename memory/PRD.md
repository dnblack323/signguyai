...
- 2026-05-17 — **Wrap Command Center Phase 1 (frontend, modular)**: Specialized workspace tied to wrap-category order items. New page `/app/frontend/src/pages/WrapCommandCenterPage.js` and 23 reusable wrap components under `/app/frontend/src/components/wrap/` (constants, header, status pipeline bar, tab nav, sidebar, generic section/checklist/AI helper/data table/empty/action group + 12 tabs: Overview, Vehicle Info, Measurements, Pricing, Design, Contract, Inspection, Production, Install, Photos & Files, Aftercare, AI Assistant). Each tab contains contextual AI helper cards next to relevant sections (not buried in one AI tab). Route added: `/orders/:orderId/items/:itemId/wrap-command-center`. `OrderDetail.js` now shows a violet "Wrap Workflow" badge + "Open Wrap Command Center" button (and dropdown item) only when item_category matches one of vehicle_wrap / vehicle_graphics / fleet_graphics / trailer_wraps / box_truck_wraps / commercial_wraps / wraps (detected via `isWrapCategory`). Phase 1 buttons are placeholders that fire sonner toasts. Non-wrap items keep their existing UI unchanged. Verified end-to-end by frontend testing agent (iteration_142.json) — 11/11 checks PASS, 0 bugs.
