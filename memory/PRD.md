...
- 2026-05-17 — **Wrap Command Center Phase 2B (Pricing & Materials + Vehicle Sync)**: Extended `routes/wrap.py` with new endpoints `POST/PUT/DELETE /api/wrap/items/{ticket_id}/materials/*`, `PUT /api/wrap/items/{ticket_id}/pricing`, `POST /api/wrap/items/{ticket_id}/recalculate`, and `POST /api/wrap/items/{ticket_id}/apply-price-to-order`. `wrap_data` doc now also stores `materials` (list with auto `total_material_cost = sqft_used * cost_per_sqft`) and `pricing` (config) + `pricing_snapshot` (computed: total_labor_cost, material_total, base_cost, markup_amount, suggested_price, per_sqft_price, quoted_price, estimated_profit, estimated_margin_percent). Three pricing methods supported: `material_labor_markup` (default), `per_sqft`, `manual`. `apply-price-to-order` pushes wrap quoted_price into `job_tickets.estimated_price + pricing_snapshot.pricing_mode='wrap'` and calls the existing `workflow_engine.update_order_progress` so `order.order_total` rolls up correctly on the main Orders Dashboard. **Vehicle sync** (folded into Phase 2B per user request): `PUT /vehicle` now also mirrors `year/make/model/trim/body_type/vehicle_color` into `job_tickets.specs.{vehicle_year, vehicle_make, vehicle_model, vehicle_trim, vehicle_body_type, vehicle_color}` (JobTicketSpecs has `Config.extra='allow'` so this is non-breaking). Frontend: PricingTab.js rewritten with method selector (3 radios), labor hours grid, fees grid, materials CRUD with inline form, profit/margin summary card, and 3 action buttons (Recalculate / Save Pricing / Apply to Order). Live local preview math mirrors backend so users see numbers update before saving. Verified end-to-end by testing_agent_v3_fork iteration_144 — backend 21/21 PASS (new pytest file `test_iteration144_wrap_phase2b.py`), frontend 100% PASS, 0 bugs.
- 2026-05-17 — **Wrap Command Center Phase 2A (Vehicle Info + Measurements persistence)**: New backend route `routes/wrap.py` registered at `/api/wrap`. Endpoints for vehicle + wrapped-areas CRUD with auto-computed coverage summary. Frontend tabs rewritten as editable forms with save status badge. Test report: iteration_143.json (backend 13/13 PASS, frontend 100% PASS).
