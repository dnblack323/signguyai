...
- 2026-05-17 — **Wrap Command Center Phase 2A (Vehicle Info + Measurements persistence)**: New backend route `routes/wrap.py` registered at `/api/wrap`. Endpoints: `GET /api/wrap/items/{ticket_id}` (auto-creates per-tenant doc on first call), `PUT /api/wrap/items/{ticket_id}/vehicle`, `POST /api/wrap/items/{ticket_id}/areas`, `PUT /api/wrap/items/{ticket_id}/areas/{area_id}`, `DELETE /api/wrap/items/{ticket_id}/areas/{area_id}`. Mongo collection `wrap_data` keyed by `(tenant_id, ticket_id)`. Every endpoint enforces wrap-category guard (`_is_wrap_category`) — non-wrap tickets get 400. Square-footage math is server-authoritative: `(width × height) / 144` for inches or `width × height` for feet; `billable = raw × (1 + waste%/100)`. Coverage summary auto-computed on every response (`total_raw_sqft, total_billable_sqft, average_waste_percent, included_count, excluded_count`). Tenant isolation enforced via current_user.tenant_id; `_id` stripped from all responses. Frontend: WrapCommandCenterPage now fetches wrap data + customer in parallel and pipes save handlers into the tabs. VehicleInfoTab is now an editable 13-field form with two boolean toggles, vehicle notes textarea, dirty-state "Unsaved changes" indicator, and a Save button that updates the new header save-status badge (idle → saving → saved → idle, plus error state). MeasurementsTab has full inline CRUD: add/edit/delete wrapped areas via a compact form with live ft²/billable preview, include/exclude toggle per row, and a 5-cell Coverage Summary card that updates live. Other 10 tabs unchanged (still Phase-1 placeholders). Verified end-to-end by testing_agent_v3_fork iteration_143 — 13/13 backend PASS, 100% frontend PASS, 0 bugs.
