...
- 2026-05-17 — **Wrap Command Center Phase 2C (Design + Contract + Approvals + Quote Draft + Pipeline)**: Extended `routes/wrap.py` with `design / contract / approvals` blocks on `wrap_data` plus a derived `pipeline_state`. Ten new endpoints: `PUT /design`, `POST /design/send-questionnaire`, `POST/PUT/DELETE /design/proofs`, `PUT /contract`, `POST /contract/action` (generate_draft / send / mark_viewed / mark_signed / store_signed), `PUT /approvals`, `POST /draft-updated-quote-message`. Mark Signed mirrors into `approvals.contract_signed`; Proof Approved mirrors into `approvals.proof_approved` (with timestamps). Approvals helper handles timestamp idempotency (true→true preserves existing `_at`, false clears it). `generate_draft` only seeds the 5-bullet default terms_summary when the field is empty. `draft-updated-quote-message` builds a real email draft from order/customer/vehicle/snapshot data (subject, body, deposit/balance halves). New frontend: `DesignTab.js` (brief + questionnaire + artwork + mockup status + proof versions CRUD + revision notes), `ContractTab.js` (status chip + 6 actions + editable fields + 7-row approval checklist + Draft Updated Quote button), `QuoteDraftModal.js` (editable subject/body + copy + close + send-later placeholder). `WrapStatusBar.js` now reads `pipeline_state` so the 15 pipeline chips light emerald/violet/slate based on real saved wrap state. Verified end-to-end by testing_agent_v3_fork iteration_145 — backend 21/21 PASS, frontend 100% PASS, 0 critical/minor issues. (a11y warning on DialogDescription fixed post-test.)
- 2026-05-17 — **Wrap Command Center Phase 2B (Pricing & Materials + Vehicle Sync)**: Materials CRUD + Pricing config/snapshot, 3 pricing methods (material_labor_markup / per_sqft / manual), apply-price-to-order rolls up via `workflow_engine.update_order_progress`. Vehicle sync writes year/make/model/trim/body_type/vehicle_color into `job_tickets.specs`. Test report: iteration_144.json — 21/21 backend, 100% frontend.
