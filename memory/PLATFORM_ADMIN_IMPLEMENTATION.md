...
```bash
curl -X POST "https://minute-calc-engine.preview.emergentagent.com/api/auth/login" \
...
### Get Tenant List:
```bash
curl -X GET "https://minute-calc-engine.preview.emergentagent.com/api/platform-admin/tenants" \
  -H "Authorization: Bearer YOUR_TOKEN"
```

### Start Impersonation:
```bash
curl -X POST "https://minute-calc-engine.preview.emergentagent.com/api/platform-admin/impersonate" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
