...
export const ProductivityKanbanView = ({ items, onOpenItem }) => {
  ...
  return (
    <div className="overflow-x-auto" data-testid="productivity-kanban-view">
      <div className="grid min-w-[1200px] gap-4" style={{ gridTemplateColumns: `repeat(${columns.length}, minmax(220px, 1fr))` }}>
        {columns.map((column) => (
          <div key={column} className="rounded-2xl border border-gray-200 bg-white p-3">
            ...
            <div className="space-y-3">
              {sortItemsByDate(groups[column] || []).map((item) => (
                <Card key={item.uid} className="border-gray-200 hover:border-violet-300 transition-colors cursor-pointer" draggable onClick={() => onOpenItem(item)} data-testid={`kanban-card-${item.uid}`}>
