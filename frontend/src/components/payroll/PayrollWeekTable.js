...
const inputClassName = 'h-12 rounded-none border-0 border-r border-slate-200 bg-transparent px-3 text-sm shadow-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500';

export const PayrollWeekTable = ({ rows, onRowChange, readOnlyLocked }) => (
    ...
    <div className="divide-y divide-slate-200">
      {rows.map((row, index) => (
        <div key={row.date} className="grid grid-cols-[140px_110px_1fr_1fr_1fr_1fr_120px_120px_120px] bg-white" data-testid={`payroll-week-row-${index}`}>
          <Input disabled={readOnlyLocked} type="date" value={row.date} onChange={(event) => onRowChange(index, 'date', event.target.value)} className={inputClassName} data-testid={`payroll-row-date-${index}`} />
          ...
          <Input disabled={readOnlyLocked} type="time" value={row.startTime} onChange={(event) => onRowChange(index, 'startTime', event.target.value)} className={inputClassName} data-testid={`payroll-row-start-${index}`} />
          <Input disabled={readOnlyLocked} type="time" value={row.lunchStart} onChange={(event) => onRowChange(index, 'lunchStart', event.target.value)} className={inputClassName} data-testid={`payroll-row-lunch-start-${index}`} />
          <Input disabled={readOnlyLocked} type="time" value={row.lunchEnd} onChange={(event) => onRowChange(index, 'lunchEnd', event.target.value)} className={inputClassName} data-testid={`payroll-row-lunch-end-${index}`} />
          <Input disabled={readOnlyLocked} type="time" value={row.endTime} onChange={(event) => onRowChange(index, 'endTime', event.target.value)} className={inputClassName} data-testid={`payroll-row-end-${index}`} />
