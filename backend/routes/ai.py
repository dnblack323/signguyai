...
# ────────── Tool registry (Pass-3 real tool calling) ──────────
#
# Each tool the assistant can run gets a JSON schema for the LLM router, an
# async executor that hits real DB/HTTP routes, and a short user-facing
# response template.

TOOL_SCHEMAS = [
    {
        "name": "navigate",
        "description": "Open a specific page inside the SignGuy AI app. Use for 'open my schedule', 'show me customers', 'go to AI tools', etc.",
        "parameters": {
            "type": "object",
            "properties": {
                "destination": {
    ...
    {
        "name": "create_task",
        "description": "Add a task to the user's task list. Use for 'add a task to call the supplier', 'remind me to order vinyl by Friday', etc.",
        "parameters": {
            "type": "object",
            "properties": {
                "title": {"type": "string"},
                "due_date_phrase": {"type": "string", "description": "Natural language like 'tomorrow', 'friday', '2026-03-04'"},
    ...
    },
    {
        ...
        "description": "Schedule an appointment / job on the calendar. Use for 'add a 2pm install for Donald tomorrow', 'schedule a measure visit Friday morning'.",
        "parameters": {
            "type": "object",
            "properties": {
                "title": {"type": "string"},
                "customer_query": {"type": "string", "description": "Customer name as the user said it; we'll look up the id."},
                "start_phrase": {"type": "string", "description": "Natural language for start time, e.g. 'tomorrow 2pm', 'friday 10am'"},
    ...
    },
    {
        ...
        "description": "Answer a number question about the shop. Use for 'how much did we make today', 'how many open orders', 'top customer this month', etc.",
        "parameters": {
            "type": "object",
            "properties": {
                "metric": {
...
def _safe_parse_datetime_phrase(phrase: str) -> Optional[str]:
    """Best-effort natural-language → ISO datetime (UTC). Handles a small
    common-vocabulary set; falls back to None if it can't tell."""
    if not phrase:
        return None
    import re as _re
    ...
    # Look for a time like "2pm" / "10:30am" / "14:00"
    m = _re.search(r"(\d{1,2})(?::(\d{2}))?\s*(am|pm)?", p)
    if m:
        hr = int(m.group(1))
...
async def _route_with_tools(message: str, tenant_id: str) -> Optional[Dict[str, Any]]:
    """Ask a fast classifier whether the user's message maps to a tool call.

    Returns ``None`` to fall through to normal chat. Otherwise returns
    ``{ "tool": <name>, "args": <dict>, "executed": <result_dict> }`` —
    the assistant endpoint will format the executed result into a brief
    reply and surface it to the UI.
    """
    if not message or len(message) > 600:
        return None

    # ── Deterministic fast-path for the most common money/metric phrasings.
    # Keeps classifier latency out of the hot path and avoids LLM flakiness
    # on questions the user expects an instant number for.
    ...
    for keywords, metric in metric_keyword_map:
        if all(k in lower for k in keywords):
            executed = await _execute_metric_query(metric, tenant_id)
            return {"tool": "query_shop_metric", "args": {"metric": metric}, "executed": executed}

    # Open-page deterministic fast path
    nav_keyword_map = [
    ...
    for phrase, dest in nav_keyword_map:
        if phrase in lower:
    ...
    classifier_prompt = (
        ...
        f"Full JSON schemas:\n{schema_json}\n\n"
        "Rules:\n"
        "1. ANY question containing 'how much', 'how many', 'total', 'revenue', "
        "'made today/week/month', 'open orders/invoices', 'top customer' → "
        "query_shop_metric. Pick the closest metric enum value.\n"
        "2. ANY explicit DO verb (open, show me, take me to, navigate) for a "
        "page → navigate.\n"
        "3. 'add a task', 'remind me to', 'remember to' → create_task.\n"
        "4. 'schedule', 'book', 'set up an appointment/meeting/install' with a "
        "date/time → create_appointment.\n"
        "5. Advice / opinion / strategy / how-to / 'should I' questions → "
        "tool: null (let the chat handle).\n\n"
    ...
    try:
        chat = LlmChat(
            ...
            system_message="You output strict JSON only. No prose, no fences.",
        ...
        logger.warning("tool router parse failed: %s", exc)
        return None
...
async def _execute_tool(tool_name: str, args: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
    """Execute a tool call and return a structured result for the UI."""
    if tool_name == "navigate":
        ...
        # Prefer a clean derived label from the destination enum over whatever
        # the model echoed back (which can be the raw user message).
        nice_label_map = {
            ...
            "schedule": "Schedule", "appointments": "Schedule", "calendar": "Schedule",
            "tasks": "Tasks", "ai_tools": "AI Tools", "financials": "Financials",
            "productivity": "Productivity", "reports": "Reports",
            "settings": "Settings", "pricing_foundation": "Pricing Foundation",
        }
        label = nice_label_map.get(dest) or args.get("spoken_label") or dest.replace("_", " ").title()
        return {
            "action_type": "navigate",
            "status": "ready",
            "path": path,
            "label": label,
            "confirm_label": f"Open {label}",
            "auto_execute": True,  # Frontend should route immediately
        }
    ...
    if tool_name == "create_task":
        ...
        due_iso = _safe_parse_datetime_phrase(args.get("due_date_phrase") or "")
        ...
        return {
            ...
            "low_risk": True,
            "auto_execute": False,  # Show confirm pill — user can edit
        }

    if tool_name == "create_appointment":
        title = (args.get("title") or "").strip() or "Appointment"
        start_iso = _safe_parse_datetime_phrase(args.get("start_phrase") or "")
        if not start_iso:
        ...
        customer_query = (args.get("customer_query") or "").strip()
        customer = None
        if customer_query:
            customer = await lookup_customer_by_name(tenant_id, customer_query)
        return {
    ...
    if tool_name == "query_shop_metric":
        ...
        return await _execute_metric_query(metric, tenant_id)

    return {"action_type": "unknown", "status": "error", "hint": "Tool not implemented"}


async def _execute_metric_query(metric: str, tenant_id: str) -> Dict[str, Any]:
    """Compute a single shop metric and return a friendly answer."""
    now = datetime.now(timezone.utc)
    ...
    if metric == "revenue_today":
        rows = await db.invoices.find(
            {"tenant_id": tenant_id, "status": "paid", "updated_at": {"$gte": today_start}},
            {"_id": 0, "grand_total": 1, "total": 1},
        ).to_list(None)
        total = sum((d.get("grand_total") or d.get("total") or 0) for d in rows)
        return {
            "action_type": "metric",
            "status": "ready",
            "metric": metric,
            ...
            "answer": f"Today's paid revenue is ${total:,.2f}.",
            "auto_execute": True,
        }
    if metric == "revenue_week":
        rows = await db.invoices.find(
            {"tenant_id": tenant_id, "status": "paid", "updated_at": {"$gte": week_start}},
            {"_id": 0, "grand_total": 1, "total": 1},
        ).to_list(None)
        total = sum((d.get("grand_total") or d.get("total") or 0) for d in rows)
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": total, "value_label": f"${total:,.2f}", "answer": f"Past 7 days: ${total:,.2f} in paid revenue.", "auto_execute": True}
    if metric == "revenue_month":
        rows = await db.invoices.find(
            {"tenant_id": tenant_id, "status": "paid", "updated_at": {"$gte": month_start}},
            {"_id": 0, "grand_total": 1, "total": 1},
        ).to_list(None)
        total = sum((d.get("grand_total") or d.get("total") or 0) for d in rows)
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": total, "value_label": f"${total:,.2f}", "answer": f"Month-to-date paid revenue: ${total:,.2f}.", "auto_execute": True}
    if metric == "open_orders":
        count = await db.jobs.count_documents(
            {"tenant_id": tenant_id, "status": {"$in": ["pending", "in_progress", "production"]}}
        )
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": count, "value_label": str(count), "answer": f"You have {count} open orders.", "auto_execute": True}
    if metric == "open_invoices_total":
        ...
        total = 0.0
        for d in rows:
            grand = d.get("grand_total") or d.get("total") or 0
            paid = d.get("amount_paid") or 0
            total += max(grand - paid, 0)
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": total, "value_label": f"${total:,.2f}", "answer": f"Outstanding invoice balance: ${total:,.2f}.", "auto_execute": True}
    if metric == "overdue_invoices_count":
        count = await db.invoices.count_documents(
            {"tenant_id": tenant_id, "status": "overdue"}
        )
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": count, "value_label": str(count), "answer": f"You have {count} overdue invoices.", "auto_execute": True}
    if metric == "customers_total":
        count = await db.customers.count_documents({"tenant_id": tenant_id})
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": count, "value_label": str(count), "answer": f"You have {count} customers in your CRM.", "auto_execute": True}
    if metric == "customers_new_30d":
        cutoff = (now - timedelta(days=30)).isoformat()
        count = await db.customers.count_documents({"tenant_id": tenant_id, "created_at": {"$gte": cutoff}})
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": count, "value_label": str(count), "answer": f"{count} new customers in the last 30 days.", "auto_execute": True}
    if metric == "stale_quotes_count":
        ...
        return {"action_type": "metric", "status": "ready", "metric": metric, "value": count, "value_label": str(count), "answer": f"{count} quotes are stale (>4 days old, not yet accepted).", "auto_execute": True}
    if metric == "top_customer_30d":
        cutoff = (now - timedelta(days=30)).isoformat()
        # Aggregate paid invoices by customer
        pipeline = [
        ...
        top = await db.invoices.aggregate(pipeline).to_list(1)
        if not top:
            return {"action_type": "metric", "status": "ready", "metric": metric, "value_label": "—", "answer": "No paid invoices in the last 30 days.", "auto_execute": True}
        t = top[0]
        return {
            "action_type": "metric",
            "status": "ready",
            "metric": metric,
            "value": t.get("total"),
            "value_label": f"{t.get('name') or 'Unknown'} — ${t.get('total', 0):,.2f}",
            "answer": f"Your top customer in the last 30 days is **{t.get('name') or 'Unknown'}** with ${t.get('total', 0):,.2f}.",
            "auto_execute": True,
        }
...
@router.post("/assistant/commit-task")
async def assistant_commit_task(
    payload: Dict[str, Any],
    current_user: UserInDB = Depends(get_current_active_user),
):
    """Persist a task proposed by the assistant. Mirrors the same insert that
    /api/tasks does so we don't drift schemas."""
...
@router.post("/assistant/commit-appointment")
async def assistant_commit_appointment(
    payload: Dict[str, Any],
    current_user: UserInDB = Depends(get_current_active_user),
):
    """Persist an appointment proposed by the assistant."""
